...
class YahooFinanceInterface:
    def __init__(self,ticker):
        ...
        self.getSummaryPage()
        self.getStatisticPage()



    def getSummaryPage(self):
        ...
        try:
            ...
            _perc = _perc.strip("()")
            live_price = [float(_p), float(_d), _perc]
    ...
    def getStatisticPage(self):
        headers = {'User-Agent': 'Chrome/93.0.4577.82'}
        url = f'https://finance.yahoo.com/quote/{self.ticker.upper()}/key-statistics?p={self.ticker.upper()}' 
        ...
        # Shares Outstanding
        try:
            path = "/html/body/div[1]/div/div/div[1]/div/div[3]/div[1]/div/div[1]/div/div/section/div[3]/div[2]/div/div[2]/div/div/table/tbody/tr[3]/td[2]"
            sharesOutstanding = (dom.xpath(path + self.pathExtension))
            self.yahoo_dict['Shares Outstanding'] = sharesOutstanding[0]
        except IndexError:
            print(f"---Failed to get Shares Outstanding---")

        #Profit Margin
        try:
            path = "/html/body/div[1]/div/div/div[1]/div/div[3]/div[1]/div/div[1]/div/div/section/div[3]/div[3]/div/div[2]/div/div/table/tbody/tr[1]/td[2]"
            profitMargin = (dom.xpath(path + self.pathExtension))
            self.yahoo_dict['Profit Margin'] = profitMargin[0]
        except IndexError:
            print("---Failed to get Profit Margin---")

        # Revenue
        try:
            path = "/html/body/div[1]/div/div/div[1]/div/div[3]/div[1]/div/div[1]/div/div/section/div[3]/div[3]/div/div[4]/div/div/table/tbody/tr[1]/td[2]"
            revenue = (dom.xpath(path + self.pathExtension))
            self.yahoo_dict['Revenue'] = revenue[0]
        except IndexError:
            print("---Failed to get Revenue---")
    ...
    def getSharesOutstanding(self):
        try:
            return self.yahoo_dict['Shares Outstanding']
        except KeyError:
            pass
